...
static void usart_start(SerialDriver *sdp, const SerialConfig *config, uint32_t irq_priority) {
    ...
    /*disable PTCR */
    u->US_PTCR = US_PTCR_RXTDIS | US_PTCR_TXTDIS;
    
    //Enable tx and tx
    u->US_CR = US_CR_RXEN | US_CR_TXEN;
    
    u->US_IER = US_IER_PARE | US_IER_FRAME | US_IER_OVRE | US_IER_RXRDY;
